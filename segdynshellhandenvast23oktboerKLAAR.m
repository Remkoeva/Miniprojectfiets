...
[a_gaslat]   = berekenactivatie2(v_gaslat);
    ...
    a_all = [a_bicfemsh, a_semimem, a_semiten, a_bicfemlh, a_gmaxsup, a_gmaxmid, a_gmaxinf, a_ilia, a_psoas, a_recfem,a_vasmed,a_vasint,a_vaslat,a_gasmed,a_gaslat];

a_heup = [a_semimem, a_semiten, a_bicfemlh, a_gmaxsup, a_gmaxmid, a_gmaxinf, a_ilia, a_psoas, a_recfem];
...
% Maak een matrix waarin elke regel [v * M] is voor die spier
Mom_x_V = [
...
vcrank = [L(1).*sin(state(1)).*state(7); L(1).*cos(state(1)).*state(7)];
evVcrank = vcrank/norm(vcrank);
...
Pknie = fip_knie*M_knie_tot;
Pheup = fip_heup*-M_heup_tot;
...
Aconstraints(9,:) = zeros(1,47);

Aconstraints(8,40:45) = [-parms.segparms.L(1)*sin(fi(1)) -parms.segparms.L(2)*sin(fi(2)) -parms.segparms.L(3)*sin(fi(3)) -parms.segparms.L(4)*sin(fi(4)) -parms.segparms.L(6)*sin(fi(6)) -parms.segparms.L(6)*sin(fi(6))]; 
Aconstraints(9,40:45) = [parms.segparms.L(1)*cos(fi(1)) parms.segparms.L(2)*cos(fi(2)) parms.segparms.L(3)*cos(fi(3)) parms.segparms.L(4)*cos(fi(4)) parms.segparms.L(5)*cos(fi(5)) parms.segparms.L(6)*cos(fi(6))]; 

Aconstraints(8,46) = 1;
Aconstraints(9,47) = 1;

Bconstraints(8,1) = parms.segparms.L(1)*cos(fi(1))*(fip(1)^2) + parms.segparms.L(2)*cos(fi(2))*(fip(2)^2) + parms.segparms.L(3)*cos(fi(3))*(fip(3)^2) + parms.segparms.L(4)*cos(fi(4))*(fip(4)^2) + parms.segparms.L(5)*cos(fi(5))*(fip(5)^2) + parms.segparms.L(6)*cos(fi(6))*(fip(6)^2);
Bconstraints(9,1) = parms.segparms.L(1)*sin(fi(1))*(fip(1)^2) + parms.segparms.L(2)*sin(fi(2))*(fip(2)^2) + parms.segparms.L(3)*sin(fi(3))*(fip(3)^2) + parms.segparms.L(4)*sin(fi(4))*(fip(4)^2) + parms.segparms.L(5)*sin(fi(5))*(fip(5)^2) + parms.segparms.L(6)*sin(fi(6))*(fip(6)^2);
...
if parms.calculate_outputs
    y = [Vnew,vspier,fip_heup,fip_knie,a_all,a_all,M_knie,Mom_x_V,fip_knie,fip_heup,v_vasmed,F_vasmed,M_spier_heup,vspier,a_heup,a_knie,Ptot,Pknie,Pheup];%,v_gmaxsup,a_all];
end
