...
state0 = [pi*-0.5 pi*0.85 pi*0.35 pi*0.70 pi*0.4 -pi*0.25 -4*pi -3.878240711416198 -3.878240711416198 -0.806805175964899 0 0 0 0 0 0 ];
...
mijnode = @(t,state) segdynshellhandenvast(t,state,u0,parms);
...
%tangentiële kracht is kracht in richting van snelheid crank
% figure
% Pcrank = -y(:,34).*-4*pi;
% plot(t,Pcrank)
...
% figure
% plot(t,y(:,151:159))
% title('spieractivaties')
% title('Spieractivaties rond heup met handen vast')
% ylim([0 1.2])
% legend('semimembranosus','semitendinosus','biceps femoris long head','gluteus maximus superior','gluteus maximus middle','gluteus maximus inferior','iliacus','psoas','rectus femoris')
...
% figure
% plot(t,Ftancrank)
% title('tangentiële kracht op trapper met handen los')
% xlabel('tijd [s]');
% ylabel('kracht [N]')
% 
% figure
% plot(t,Fradcrank)
% title('radiale kracht op trapper met handen los')
% xlabel('tijd [s]');
% ylabel('kracht [N]')
...
% Pheupvast = y(:,169);
% tPgewricht = t;
% save('gewrichtsvermogensvast','Pknievast','Pheupvast','tPgewricht')

Pcrankvast = state(:,7).*-y(:,34);
tcrankvast = t;

save('Pcrankvast','Pcrankvast','tcrankvast')
