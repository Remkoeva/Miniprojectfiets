...
%Moment =  59.8495;
Moment = -6.547946504007387; %-12.454934381976590; %71.660921167714820;
...
P_gmax = (Moment / d_Glut_max_heup) * v_gmax;
P_spier = [abs(P_bicfemlh), abs(P_semimem),  abs(P_semiten), abs(P_gmax)];
...
elseif index == 4
    ...
    a_semimem  = 0;
    a_semiten  = 0;
    a_gmaxtot = abs(Moment / ((Fmaxfv_gmaxsup+Fmaxfv_gmaxinf+Fmaxfv_gmaxmid) * d_Glut_max_heup));
...
if parms.calculate_outputs
    y = [Vnew,vspier,fip_heup,fip_knie,Mcontrol,potmomext,M_spier_heup,potmomflex,potmomext,a_all];%,v_gmaxsup,a_all];
end
