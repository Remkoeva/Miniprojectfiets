...
if (first)  
   ...
   lu = length(find(~K));
   if (nargin > 4) %constraints present!
       [nr_bcons,nc_bcons] = size(b_cons);
       [nr_Acons,nc_Acons]=size(A_cons);
       if nc_bcons ~= 1
           disp('Number columns in b_cons should be 1');
           return
       elseif nc_Acons~=7*n+5
           err = sprintf('Number of columns in A_cons is incorrect, should be %d',7*n+5);
           disp(err);
           return
       elseif nr_Acons ~= nr_bcons
           disp('Number of rows in A_cons and b_cons are different');
           return
       end          
   else
       nr_bcons = 0;
   ...
   if (lu > 3*n + nr_bcons)
      err=sprintf('%d unknowns found: at most %d elements in K may be zero!',lu,3*n+nr_bcons); 
      disp(err);
      return;
   end    
