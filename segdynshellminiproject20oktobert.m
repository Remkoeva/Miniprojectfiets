...
%TEST VOOR SPIERKRACHT BICFEMSH
...
%Moment van reactiekracht in heup counteren
Aconstraints(7,:) = [zeros(1,4) 0.2214 zeros(1,32) 1 zeros(1,9)];
Bconstraints(7,1) = 0;
...
if parms.calculate_outputs
    y = [Vnew,vspier,M_knie_tot,Pknie,Pheup];%M_heup_tot,M_spier_heup,a_gmaxsup,F_gmaxsup, d_Glut_max_heup,Fmax_gmaxsup, a_gmaxsup, v_gmaxsup, vmax_gmaxsup, opt_fl_gmaxsup, fl_gmaxsup];%glob_ori_vaslat(1),glob_ori_vaslat(2),glob_wrap1_vaslat(1),glob_wrap1_vaslat(2),glob_ins_vaslat(1),glob_ins_vaslat(2)];%rH(1),rH(2),rPel(1),rPel(2)];
end
