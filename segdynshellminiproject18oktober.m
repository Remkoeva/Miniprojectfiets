...
rPat = [L(1)*cos(fi(1)) + L(2)*cos(fi(2)) + L(3)*cos(fi(3)) + Knie_Patella(1); L(1)*sin(fi(1)) + L(2)*sin(fi(2)) + L(3)*sin(fi(3)) + Knie_Patella(2)];
...
Fmax_gmaxinf = 368;
pees_sl_gmaxin = 0.1450;
opt_fl_gmaxin = 0.1440;
vmax_gmaxin = vmax*opt_fl_gmaxin;
...
% l_vasmed = norm(glob_wrap1_vasmed - glob_ori_vasmed) + norm(glob_wrap2_vasmed - glob_wrap1_vasmed) + norm(glob_ins_vasmed - glob_wrap2_vasmed);
% 
% % Vastus intermedius
...
% l_vaslat = norm(glob_wrap1_vaslat - glob_ori_vaslat) + norm(glob_wrap2_vaslat - glob_wrap1_vaslat) + norm(glob_ins_vaslat - glob_wrap2_vaslat);
% 
...
mom_bicfemsh = cross2d(r_ori_bicfemsh_knie,(F_bicfemsh*e_inor_bicfemsh));
...
if parms.calculate_outputs
    y = [Vnew, l_bicfemsh,v_bicfemsh,F1_bicfemsh,d_bicfemsh,a,mom_bicfemsh];
end
