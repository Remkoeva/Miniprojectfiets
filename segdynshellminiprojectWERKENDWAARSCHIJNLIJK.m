...
%Moment =  59.8495;
Moment = -12.454934381976590;%-6.547946504007387; %-12.454934381976590; %71.660921167714820;%-6.547946504007387;
...
% % 2. Sorteer op vermogen (hoogste eerst)
% [Ps_sorted, idx_sorted] = sort(P_spier, 'descend');
% 
% % 3. Initialiseer activaties
% a = zeros(1,6);
% 
% % 4. Loop over de spieren in volgorde van bijdrage
% RemainingMoment = abs(Moment);
% 
% for i = 1:length(idx_sorted)
%     spier_idx = idx_sorted(i);
% 
%     % Bereken max kracht voor deze spier
%     switch spier_idx
%         case 1
%             Fmaxfv = Fmaxfv_bicfemlh;
%             d = d_Biceps_fem_heup;
%         case 2
%             Fmaxfv = Fmaxfv_semimem;
%             d = d_Biceps_fem_heup;
%         case 3
%             Fmaxfv = Fmaxfv_semiten;
%             d = d_Biceps_fem_heup;
%         case 4
%             Fmaxfv = Fmaxfv_gmaxsup;
%             d = d_Glut_max_heup;
%         case 5
%             Fmaxfv = Fmaxfv_gmaxmid;
%             d = d_Glut_max_heup;
%         case 6
%             Fmaxfv = Fmaxfv_gmaxinf;
%             d = d_Glut_max_heup;
%     end
% 
%     % 5. Bepaal benodigde activatie voor resterend moment
%     a_needed = RemainingMoment / abs((Fmaxfv * d));
% 
%     if a_needed <= 1
%         % Deze spier kan alles leveren → done
%         a(spier_idx) = a_needed;
%         RemainingMoment = 0;
%         break;
%     else
%         % Spier levert maximaal wat hij kan (a = 1)
%         a(spier_idx) = 1;
%         % Verminder resterend moment
%         RemainingMoment = RemainingMoment - (Fmaxfv * d);
%     end
% end

% 6. Zet activaties in namen (optioneel)
a_bicfemlh = a(1);
a_semimem  = a(2);
a_semiten  = a(3);
a_gmaxsup  = a(4);
...
% 
%     Fmaxfv_bicfemlh = berekenmaxspierkracht(Fmax_bicfemlh,v_bicfemlh,vmax_bicfemlh,opt_fl_bicfemlh,fl_bicfemlh);
%     a_bicfemlh = abs(Moment / (Fmaxfv_bicfemlh * d_Biceps_fem_heup));
%     a_semimem  = 0;
%     a_semiten  = 0;
%     a_gmaxsup  = 0;
%     a_gmaxmid  = 0;
...
%     Fmaxfv_semimem = berekenmaxspierkracht(Fmax_semimem,v_semimem,vmax_semimem,opt_fl_semimem,fl_semimem);
%     a_bicfemlh = 0;
%     a_semimem  = abs(Moment / (Fmaxfv_semimem * d_Biceps_fem_heup));
%     a_semiten  = 0;
%     a_gmaxsup  = 0;
%     a_gmaxmid  = 0;
%     a_gmaxinf  = 0;
...
%     a_bicfemlh = 0;
%     a_semimem  = 0;
%     a_semiten  = abs(Moment / (Fmaxfv_semiten * d_Biceps_fem_heup));
%     a_gmaxsup  = 0;
%     a_gmaxmid  = 0;
%     a_gmaxinf  = 0;
%     a_ilia     = 0;
...
% 
%       Fmaxgmax = Fmaxfv_gmaxsup+Fmaxfv_gmaxmid+Fmaxfv_gmaxinf;
%     Momgmaxmax = abs(Fmaxgmax*d_Glut_max_heup);
% 
% 
%     a_bicfemlh = 0;
%     a_semimem  = 0;
%     a_semiten  = 0;
%     a_gmaxtot = abs(Moment / Momgmaxmax);
%     % a_gmaxsup  = (Moment/Momgmaxmax)*(Fmaxfv_gmaxsup/Fmaxgmax);
%     % a_gmaxmid  = (Moment/Momgmaxmax)*(Fmaxfv_gmaxmid/Fmaxgmax);
%     % a_gmaxinf  = (Moment/Momgmaxmax)*(Fmaxfv_gmaxinf/Fmaxgmax);
%     a_ilia     = 0;
%     a_psoas    = 0;
%     a_recfem   = 0;
% 
% 
% 
%     a_gmaxsup = a_gmaxtot *(Fmaxfv_gmaxsup/Fmaxgmax);
%       a_gmaxmid = a_gmaxtot *(Fmaxfv_gmaxmid/Fmaxgmax);
%         a_gmaxinf = a_gmaxtot *(Fmaxfv_gmaxinf/Fmaxgmax);
%     end
% 
% 
% 
...
% 
% P_spier = [P_ilia,P_psoas,P_recfem];
% 
% [M,index] = max(P_spier);
% if index == 1
%     % Iliacus actief
...
%     a_gmaxmid  = 0;
%     a_gmaxinf  = 0;
%     a_ilia     = abs(Moment / (Fmaxfv_ilia * d_ilia_heup));
%     a_psoas    = 0;
%     a_recfem   = 0;
% 
% elseif index == 2
...
%     a_gmaxinf  = 0;
%     a_ilia     = 0;
%     a_psoas    = abs(Moment / (Fmaxfv_psoas * d_ilia_heup));
%     a_recfem   = 0;
% 
% elseif index == 3
...
%     a_ilia     = 0;
%     a_psoas    = 0;
%     a_recfem   = abs(Moment / (Fmaxfv_recfem * d_Recfem_heup));
% 
% end
...
if parms.calculate_outputs
    y = [Vnew,vspier,fip_heup,fip_knie,Mcontrol,potmomext,potmomflex,potmomext,M_spier_heup,a_tot_knie,a_all,P_spier];%,v_gmaxsup,a_all];
end
